...
def p2pChat(connectionSocket, userName, address):
    ...
    connectionSocket.send("Who would you like to chat with?".encode('utf-8')) # ask who the client wants to chat with
    ...
    while otherClient not in clientDictionary.values() :#while a the other client isn't a username in the clientDictionary
        connectionSocket.send("That user doesn't seem to be avaliable?".encode('utf-8')) #ask who the client wants to chat with
    ...
    for otherclientAdr in clientDictionary.keys(): #look for the other client's info in clientDictionary
        if otherClient == clientDictionary.get(otherclientAdr): #if the otherClient's username is in the clientDictionery
            ...
            message = "Establishing connection..."+str(address)+"&"+str(otherclientAdr) #send the current client and other clients address tuples as strings
            connectionSocket.send(message.encode('utf-8')) #ask who the client wants to chat with
    
    while connectionSocket.recv(1024).decode('utf-8') != "exit": #stay here while the clients are in the chat room and someone types 'exit'
        continue
    ...
    while len(clientDictionary) > 0: #while there are client's on the server
                ...
                if str(option) == '1':
                    ...
                    while len(clientDictionary) > 0:
                        ...
                        if str(option) == '1':
                            connectionSocket.send("You are now appear online and are visible to other members on the server".encode('utf-8'))
        ...
        try:
            ...
            if  str(option) == '1':
                ...
                if count < 2 and clientStatusDictionary.get(userName) == "online":
                    connectionSocket.send("No other server members online ".encode('utf-8'))
                else:
                ...
                connectionSocket.send("disconnect".encode('utf-8'))
...
def handleClient(connectionSocket, address): #handle the clients' connections to the server and proved services
    ...
    try:
        ...
        if clientStatusDictionary.get(userName) == "online": #ERRORS?
            broadcastMessage(f"{userName} has left the server".encode('utf-8'))
